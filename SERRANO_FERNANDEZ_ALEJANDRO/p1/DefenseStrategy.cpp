...
{
	float a = pow(abs(celdaActual.row() - (*objetoActual)->position.x),2);
	float b = pow(abs(celdaActual.col() - (*objetoActual)->position.y),2);
    	return (sqrt(a+b));   	
}

float distancia(Celda celdaActual,std::list<Object*>::iterator objetoActual)
{
	float a = pow(abs(celdaActual.row() - (*objetoActual)->position.x),2);
	float b = pow(abs(celdaActual.col() - (*objetoActual)->position.y),2);
    	return (sqrt(a+b));   	
	...
	, float mapWidth, float mapHeight, List<Object*> obstacles, List<Defense*> defenses, std::list<Defense*>::iterator defensa_actual)
{
	...
	//No sale del mapa
	if( (celdaActual.row() + (*defensa_actual)->radio) > mapWidth) es_factible = false;
	else if( (celdaActual.col() + (*defensa_actual)->radio) > mapHeight) es_factible = false;
